...
-- 2. 创建表
DROP TABLE IF EXISTS jtp_oms_dwd.dwd_oms_order_detail_incr;
CREATE TABLE jtp_oms_dwd.dwd_oms_order_detail_incr
(
    ...
    STORED AS TEXTFILE
    LOCATION 'hdfs://node101:8020/user/spark/warehouse/jtp_oms_dwd/dwd_oms_order_detail_incr'
;


-- 查看分区和数据
SHOW PARTITIONS jtp_oms_dwd.dwd_oms_order_detail_incr;
SELECT * FROM jtp_oms_dwd.dwd_oms_order_detail_incr LIMIT 10;
SELECT count(id) AS cnt FROM jtp_oms_dwd.dwd_oms_order_detail_incr ;
...
INSERT OVERWRITE TABLE jtp_oms_dwd.dwd_oms_order_detail_incr PARTITION (dt)
SELECT
    id
