...
SELECT dt, coupon_get_count, coupon_used_count, coupon_used_ratio
FROM jtp_oms_warehouse.ads_coupon_ratio_report
UNION
SELECT '2024-12-31'                                                     AS dt
...
SELECT '2025-01-01'                                     AS dt
     ...
     , concat(
            round(
                        sum(if(metrics_type = 'used', metrics_count, 0)) /
                        sum(if(metrics_type = 'get', metrics_count, 0))
                , 4
                ) * 100
    , '%'
    )                                                   AS conpon_used_rate
FROM (
         ...
         SELECT '2025-01-01' AS dt
              ...
              , count(id)    AS metrics_count
         FROM jtp_oms_warehouse.dwd_coupon_used_incr) t1
;