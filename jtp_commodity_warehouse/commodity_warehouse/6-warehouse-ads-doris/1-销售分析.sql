...
SELECT
    ...
    ROUND(
                COUNT(DISTINCT CASE WHEN action_type IN ('view', 'view_micro')
...
FROM (
         ...
         SELECT
             dt, sku_id, user_id, action_type, stay_duration,
...
FROM
    hive_catalog.jtp_commodity_warehouse.dwd_aggregated_wide
GROUP BY
    dts,
    sku_id,
    product_id,
...
WITH color_stats AS (
    SELECT
        color_category AS color_category,
    ...
    GROUP BY color_category
     ...
     total_stats AS (
         -- 单独计算总支付金额和总支付买家数
         SELECT
             SUM(pay_count) AS sum_pay_money,
