...
) ENGINE = OLAP DUPLICATE KEY (dt,sku_id)
    COMMENT '客群分析'
...
SELECT *
FROM jtp_gd03_warehouse.service_experience_analysis;
...
(
    ...
    -- 分区与存储
    dt_part               date COMMENT '分区日期（YYYY-MM-DD）'
) ENGINE = OLAP DUPLICATE KEY (dt,sku_id)
    COMMENT '客群分析'
    DISTRIBUTED BY HASH(dt,sku_id) BUCKETS 10
    PROPERTIES (
...
SELECT *
FROM jtp_gd03_warehouse.service_experience_analysis2;
...
-- 再执行插入语句的时候遇到了数据质量 因为使用了CAST(dts AS DATE)
...
-- 修改表结构
ALTER TABLE jtp_gd03_warehouse.service_experience_analysis3
    MODIFY COLUMN review_content_text VARCHAR(2048);

INSERT INTO jtp_gd03_warehouse.service_experience_analysis3
SELECT
    -- 基础维度
    CAST(STR_TO_DATE(dts, '%Y-%m-%d') AS DATE) AS dt,
    sku_id,
    -- 评价内容分析
    CONCAT(
            '红色:', COUNT(CASE WHEN review_content RLIKE '红色|RED' THEN 1 END), ',',
    ...
    LEFT(GROUP_CONCAT(review_content, ' | '), 500) AS review_content_text,
    -- 分区与存储
    DATE_FORMAT(STR_TO_DATE(dts, '%Y-%m-%d'), '%Y-%m-%d') AS dt_part
FROM hive_catalog.jtp_commodity_warehouse.dwd_aggregated_wide
WHERE
    sku_id IS NOT NULL AND
  -- 修正：使用STR_TO_DATE并过滤NULL值，确保只有有效的日期才会被处理
    STR_TO_DATE(dts, '%Y-%m-%d') IS NOT NULL
GROUP BY
    CAST(STR_TO_DATE(dts, '%Y-%m-%d') AS DATE),
    sku_id,
    DATE_FORMAT(STR_TO_DATE(dts, '%Y-%m-%d'), '%Y-%m-%d');

SELECT *
FROM jtp_gd03_warehouse.service_experience_analysis3;
