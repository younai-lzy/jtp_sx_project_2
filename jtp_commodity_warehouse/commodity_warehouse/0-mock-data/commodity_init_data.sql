...
-- 8. 内容数据表 (ods_content_data_full)
-- 作用：存储与商品相关的营销内容数据，用于分析内容营销效果。
-- CREATE TABLE `ods_content_data_full`
-- (
--     `id`           INT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '自增主键',
--     `content_id`   INT UNSIGNED NOT NULL COMMENT '内容ID',
--     `content_type` VARCHAR(50) NOT NULL COMMENT '内容类型：live（直播）, short_video（短视频）, graphic（图文）',
--     `sku_id`       INT UNSIGNED NOT NULL COMMENT '关联的SKU ID',
--     `view_count`   INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '内容观看/浏览量',
--     `like_count`   INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '内容点赞数',
--     `share_count`  INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '内容分享数',
--     `create_time`  DATETIME    NOT NULL COMMENT '内容创建时间',
--     PRIMARY KEY (`id`),
--     KEY `idx_sku_id` (`sku_id`)
-- ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='内容数据表';
...
-- 定义分隔符，以便在存储过程中使用分号
DELIMITER $$
-- 创建存储过程，用于向所有 ODS 表插入模拟数据
CREATE
PROCEDURE `insert_ods_mock_data`()
BEGIN
    -- 定义循环计数器和ID生成器
    DECLARE i INT DEFAULT 0;
DECLARE sku_id_counter INT DEFAULT 10000;
DECLARE user_id_counter INT DEFAULT 100000;
DECLARE order_id_counter INT DEFAULT 1000000;
DECLARE content_id_counter INT DEFAULT 1000;
DECLARE start_time DATETIME DEFAULT '2024-01-01 00:00:00';

    -- 1. 生成 ods_product_info_full 数据 (10万条)
WHILE i < 100000 DO
INSERT INTO `ods_product_info_full` (`sku_id`, `product_id`, `product_name`, `category_id`, `category_name`, `brand_id`,
                                     `brand_name`, `original_price`, `create_time`)
VALUES (sku_id_counter + i, sku_id_counter + i, CONCAT('商品_', i), FLOOR(1 + RAND() * 10),
        CONCAT('类目_', FLOOR(1 + RAND() * 10)), FLOOR(1 + RAND() * 20), CONCAT('品牌_', FLOOR(1 + RAND() * 20)),
        100 + RAND() * 900, DATE_ADD(start_time, INTERVAL i MINUTE));
SET i = i + 1;
END WHILE;

    -- 重置计数器
SET i = 0;
-- 2. 生成 ods_user_full 数据 (10万条)
WHILE i < 100000 DO
        -- 随机生成出生日期，用于计算年龄
        DECLARE rand_birth_date DATE;
SET rand_birth_date = DATE_ADD('1970-01-01', INTERVAL FLOOR(RAND() * (365 * 50)) DAY);
INSERT INTO `ods_user_full` (`user_id`, `username`, `registration_time`, `gender`, `birth_date`, `city`, `hobby`)
VALUES (user_id_counter + i, CONCAT('user_', i), DATE_ADD(start_time, INTERVAL FLOOR(RAND() * 365) DAY),
        CASE FLOOR(RAND() * 2) WHEN 0 THEN '男' ELSE '女' END,
        rand_birth_date,
        CONCAT('城市_', FLOOR(1 + RAND() * 50)),
        CASE FLOOR(RAND() * 5)
            WHEN 0 THEN '运动'
            WHEN 1 THEN '阅读'
            WHEN 2 THEN '音乐'
...
END WHILE;
    ...
    -- 重置计数器
SET i = 0;
-- 3. 生成 ods_user_action_log 数据 (10万条)
WHILE i < 100000 DO
INSERT INTO `ods_user_action_log` (`session_id`, `user_id`, `sku_id`, `action_type`, `source_channel`, `log_timestamp`)
VALUES (UUID(), user_id_counter + FLOOR(RAND() * 100000), sku_id_counter + FLOOR(RAND() * 100000),
        CASE FLOOR(RAND() * 5)
            WHEN 0 THEN 'view'
            WHEN 1 THEN 'click'
            ...
            WHEN 3 THEN 'search'
            ELSE 'buy' END,
        CASE FLOOR(RAND() * 4) WHEN 0 THEN '自然搜索' WHEN 1 THEN '广告' WHEN 2 THEN '推荐' ELSE '社交媒体' END,
        UNIX_TIMESTAMP() - FLOOR(RAND() * 3600 * 24 * 30));
SET i = i + 1;
END WHILE;

    -- 重置计数器
SET i = 0;
-- 4. 生成 ods_order_full 数据 (10万条)
WHILE i < 100000 DO
INSERT INTO `ods_order_full` (`order_id`, `user_id`, `total_amount`, `status`, `create_time`, `pay_time`)
VALUES (order_id_counter + i, user_id_counter + FLOOR(RAND() * 100000), 100 + RAND() * 900,
        CASE FLOOR(RAND() * 3) WHEN 0 THEN 'paid' WHEN 1 THEN 'unpaid' ELSE 'closed' END,
        DATE_ADD(start_time, INTERVAL FLOOR(RAND() * 30) DAY),
        CASE FLOOR(RAND() * 2) WHEN 0 THEN NULL ELSE DATE_ADD(start_time, INTERVAL FLOOR(RAND() * 30) DAY) END);
SET i = i + 1;
END WHILE;

    -- 重置计数器
SET i = 0;
-- 5. 生成 ods_order_detail_full 数据 (10万条)
WHILE i < 100000 DO
INSERT INTO `ods_order_detail_full` (`order_id`, `sku_id`, `buy_num`, `item_price`)
VALUES (order_id_counter + FLOOR(RAND() * 100000), sku_id_counter + FLOOR(RAND() * 100000), FLOOR(1 + RAND() * 5),
        100 + RAND() * 900);
SET i = i + 1;
END WHILE;
    ...
    -- 重置计数器
SET i = 0;
-- 6. 生成 ods_price_trend_log 数据 (10万条)
WHILE i < 100000 DO
SET @old_price = 100 + RAND() * 900;
INSERT INTO `ods_price_trend_log` (`sku_id`, `price_before`, `price_after`, `change_time`)
VALUES (sku_id_counter + FLOOR(RAND() * 100000), @old_price, @old_price * (1 + (RAND() - 0.5) * 0.2),
...
END WHILE;
    ...
    -- 重置计数器
SET i = 0;
-- 7. 生成 ods_product_review_full 数据 (10万条)
WHILE i < 100000 DO
INSERT INTO `ods_product_review_full` (`sku_id`, `user_id`, `order_id`, `score`, `review_content`, `review_time`,
                                       `is_positive`)
VALUES (sku_id_counter + FLOOR(RAND() * 100000), user_id_counter + FLOOR(RAND() * 100000),
        order_id_counter + FLOOR(RAND() * 100000), FLOOR(1 + RAND() * 5), '这是一个评价内容',
        DATE_ADD('2024-01-01 00:00:00', INTERVAL FLOOR(RAND() * 365) DAY),
...
END WHILE;
    ...
    -- 重置计数器
SET i = 0;
-- 8. 生成 ods_content_data_full 数据 (10万条)
WHILE i < 100000 DO
INSERT INTO `ods_content_data_full` (`content_id`, `content_type`, `sku_id`, `view_count`, `like_count`, `share_count`,
                                     `create_time`)
VALUES (content_id_counter + i,
        CASE FLOOR(RAND() * 3) WHEN 0 THEN 'live' WHEN 1 THEN 'short_video' ELSE 'graphic' END,
        sku_id_counter + FLOOR(RAND() * 100000), FLOOR(RAND() * 10000), FLOOR(RAND() * 1000), FLOOR(RAND() * 100),
        DATE_ADD('2024-01-01 00:00:00', INTERVAL FLOOR(RAND() * 365) DAY));
SET i = i + 1;
END WHILE;
